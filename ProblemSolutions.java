...
public class ProblemSolutions {
    ...
    public static void selectionSort(int[] values, boolean ascending) {
        ...
        int minMax = 0; // the lowest/highest value which is being compared in values
        int temp = 0; // for swapping values
        int jndex = 0; // index position for the inner loop
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            minMax = values[i];
            temp = values[i];
            jndex = 0; // reset values each loop

            // based on ascending, look for min/Max on the unsorted side of values
            if (ascending) // min
                for (int j = i; j < n; j++) {
                    if (minMax > values[j]) {
                        minMax = values[j];
            ...
            // only swap values if the minMax value was updated, avoiding unnecessary swaps
            if (minMax != values[i]) {
                values[i] = minMax;
                values[jndex] = temp;
            }
    ...
    private void mergeDivisbleByKFirst(int arr[], int b, int left, int mid, int right) {
    ...
    public static int numRescueSleds(int[] people, int limit) {
        ...
        // loop through people, grouping them together into sleds as we traverse the loop.
        // if a group's weight exceeds the limit, increment sleds & begin to make a new group
        ...
        // account for any leftover people that didn't get added in
        return (sum != 0) ? sleds + 1 : sleds;
    }
